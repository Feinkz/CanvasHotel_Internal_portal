...
<div class='content'>
    <form action="../source/fb_rate.php" method="post">
...
<?php
    ...
    echo "<H1>Рейтинг отдела F&B </H1>";

echo "<TABLE class='table_blur'>";
echo "<TR>";
echo "<TH>Имя Фамилия Официанта </TH>";
echo "<TH>Средний чек</TH>";
echo "<TH>Количество Чеков</TH>";
echo "<TH>Сумма</TH>";

 $res=queryMysql("SELECT * FROM micros_employee");
 $num=$res->num_rows;
for ($j=0; $j<$num; $j++){
   echo "<TR>";
    $sum=0;
    ...
    echo "<TD>".$r['Name']."</TD>";
    $stid=queryOCI("SELECT  checktotal, employeeid FROM LOCATION_ACTIVITY_DB.guest_check_hist where  to_date((to_char(closedatetime, 'MM.YYYY')), 'MM.YY') = to_date('".$oci_date."','MM.YY') AND employeeid=".$r['emplid']."AND numguests<5 AND revenuecenterid=1151451");
    oci_execute($stid);
    $sum=0;
    $count=0;
    while ($row= oci_fetch_array($stid, OCI_BOTH)){
        $sum=$sum+$row['CHECKTOTAL'];
        $count++;
        }
    echo "<TD>".$mid=round(round($sum)/$count)."</TD>";
    echo "<TD>".$count."</TD>";
    echo "<TD>".$sum."</TD>";
                }
